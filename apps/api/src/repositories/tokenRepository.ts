...
export async function createRefreshToken(userId: number, token: string, expiresAt: Date, userAgent?: string, ipAddress?: string) {
...
export async function getRefreshTokenDatabaseValues(token: string): Promise<RefreshToken| undefined> {
  const tokenHash = hashToken(token);
  const { rows } = await pool.query(
    `SELECT * FROM refresh_tokens WHERE token_hash = $1`,
    [tokenHash]
