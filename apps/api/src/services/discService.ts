...
export async function getDiscsBySeller(
    sellerId: number,
): Promise<PublicDisc[]> {
...
export async function updateDiscForSeller(
    ...
    if (!disc) {
        throw new NotFoundError(
            "Disc not found or you don't have permission to update it",
        );
    }
    return toPublicDisc(disc);
}
...
export async function removeDisc(id: number, sellerId: number): Promise<void> {
    ...
    if (!deleted) {
        throw new NotFoundError(
            "Disc not found or you don't have permission to delete it",
        );
    }
}

export async function searchDiscsWithFilters(
    filters: DiscSearchFilters,
): Promise<{
    discs: PublicDisc[];
    total: number;
}> {
