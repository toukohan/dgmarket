...
import { createRefreshToken, findRefreshToken, revokeRefreshToken } from "../../src/repositories/tokenRepository";
...
describe.sequential("token repository", () => {
    ...
    it("stores refresh token with users id", async () => {
        ...
        await createRefreshToken(user.id, refreshToken, new Date(Date.now() + 60000))
